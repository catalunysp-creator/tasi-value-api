...
async def extract_text(file: UploadFile) -> str:
    ...
    if name.endswith(".pdf"):
        ...
        try:
            ...
            try:
                df = pd.read_excel(io.BytesIO(content), engine="openpyxl", na_filter=False, dtype=str)
                return df.to_string()
            except Exception as e2:
                return f"تعذر قراءة ملف Excel: {str(e2)}"
        ...
        try:
            df = pd.read_csv(io.BytesIO(content), na_filter=False)
            return df.to_string()
        except Exception as e:
            return f"تعذر قراءة ملف CSV: {str(e)}"
...
async def analyze_and_store(
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"فشل استخراج النص: {type(e).__name__}: {str(e)}")

    if not text.strip():
        raise HTTPException(status_code=422, detail="لم يتم استخراج نص من الملف")
...
""")
        ...
        raise HTTPException(status_code=502, detail=f"خطأ في Gemini API: {type(e).__name__}: {str(e)}")
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"خطأ في حفظ البيانات: {type(e).__name__}: {str(e)}")
